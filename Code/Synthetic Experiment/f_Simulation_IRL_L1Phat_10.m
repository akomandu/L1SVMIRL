function [] = f_Simulation_IRL_L1Phat_10(Pcell,PHatcell, S,A,m,nsamp)
% A = 4;
% S = 4;
n1 = 50;
n2 =10000;
...
for kk = 1:m
    ...
    variables R(S) rho
    % minimize( 1/2*R.'*R + C * sum((-F{2}-F{3}-F{4}-F{5})*R) )
    minimize( lambda*norm(R,1) + sum(max(max(max(max(max(max(max(max(-F{2}*R,-F{3}*R),-F{4}*R),-F{5}*R),-F{6}*R),-F{7}*R),-F{8}*R),-F{9}*R),-F{10}*R)) )
    % minimize( 1/2*R.'*R -rho)
    subject to
    F{2}*R >= zeros(S,1)
    F{3}*R >= zeros(S,1)
